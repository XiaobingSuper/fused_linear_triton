...
class MLP_ref(torch.nn.Module):
    def __init__(self):
        ...
        self.linear_0 = torch.nn.Linear(in_features=64, out_features=2, bias=True)
...
class MLP(torch.nn.Module):
    def __init__(self):
        ...
        self.linear_0 = triton_ops.FusedLinear(64, 2, bias=True)
...
mlp_ref.linear_0.weight.data = mlp.linear_0.weight.data.clone()
if mlp_ref.linear_0.bias is not None:
    mlp_ref.linear_0.bias.data = mlp.linear_0.bias.data.clone()

 
mlp_ref = mlp_ref.train()
mlp.train()

input_mat_ref = torch.rand(1048576, 64).cuda().fill_(1.0)
...
grad_ref = torch.rand(1048576, 2).cuda().fill_(1.0)
grad = grad_ref.clone().detach().fill_(1.0)
...
print(torch.max((mlp_ref.linear_0.weight.grad-mlp.linear_0.weight.grad).abs()))
